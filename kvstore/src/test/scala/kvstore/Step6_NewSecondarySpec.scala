...
class Step6_NewSecondarySpec extends TestKit(ActorSystem("Step6NewSecondarySpec"))
  ...
  test("case4: Primary must start replication to new replicas") {
    val arbiter = TestProbe()
  ...
  test("case5: Primary must stop replication to removed replicas and stop Replicator") {
    val arbiter = TestProbe()
  ...
  test("case6: Primary must stop replication to removed replicas and waive their outstanding acknowledgements") {
    val arbiter = TestProbe()
